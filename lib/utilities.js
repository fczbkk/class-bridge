...
function parseRegexp(input, class_names) {
  var result = { truthy: [], falsy: [] };

  class_names.forEach(function (item) {
    var target = input.test(item) ? result.truthy : result.falsy;
    target.push(item);
  });
...
function parseClassList(input, namespace) {
  var class_names = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
